      ...
      {prescriptions.length === 0 ? (
        ...
        <div className="space-y-4 max-h-96 overflow-y-auto">
          {prescriptions.map((prescription) => (
            <Card
              key={prescription.id}
              className={`cursor-pointer transition-all duration-200 hover:shadow-lg border border-gray-300 dark:border-gray-700 bg-gray-50 text-black dark:bg-zinc-800 dark:text-white ${
                selectedPrescriptionId === prescription.id
                  ? 'ring-2 ring-black dark:ring-white bg-gray-100 dark:bg-zinc-700'
                  : 'hover:bg-gray-100 dark:hover:bg-zinc-700'
              }`}
              onClick={() => onSelectPrescription(prescription)}
            >
              <CardHeader className="pb-3 text-black dark:text-white">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <CardTitle className="text-lg font-semibold text-black dark:text-white truncate">
                      {prescription.fileName}
                    </CardTitle>
                    <div className="flex items-center space-x-4 mt-2 text-sm text-gray-800 dark:text-gray-300">
              ...
              <CardContent className="pt-0 text-black dark:text-gray-200">
                <div className="text-sm text-gray-900 dark:text-gray-200 line-clamp-3">
